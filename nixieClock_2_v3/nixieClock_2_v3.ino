/*
  Скетч к проекту "Часы на ГРИ версия 3"
  платы тут https://easyeda.com/technotrasher/nixie_clock_pcb_in14
  используйте доп. плату на транзисторных ключах. не будет засветов цифр и лишних цифр.
  переменный резистор не менее 0.5 Ватт!!!!  
  для плат ИН14 и ИН 12 - изменить в строках 178 - 180
  @Sergo_ST  огромная благодарность!!! взят его вариант реализации модулей для датчиков и часов.
  переписан опрос датчика СО2.
*/
...
#define DOT_BRIGHT 40    // яркость точки дневная (1 - 255) 80
#define DOT_BRIGHT_N 25  // яркость точки ночная (1 - 255)

#define BACKL_BRIGHT 50    // макс. яркость подсветки дневная (0 - 255) 80
#define BACKL_BRIGHT_N 40  // макс. яркость подсветки ночная (0 - 255, 0 - подсветка выключена)
