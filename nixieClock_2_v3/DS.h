...
void readTempDS(void)
{
  ...
  if (!tempSens.temp) { //если порт не инициализирован
    tempSens.temp = 1; //устанавливаем флаг инициализации
  ...
  switch (typeDS) {
    case DS18S20_ADDR: tempSens.temp = raw * 50; break; //переводим в температуру для DS18S20
    case DS18B20_ADDR: tempSens.temp = (raw * 100) >> 4; break; //переводим в температуру для DS18B20
  }
